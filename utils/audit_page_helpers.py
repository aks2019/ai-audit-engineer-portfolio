"""Reusable RAG Audit Report, Engagement Selector & Draft Review helpers for all Detection pages.
"""
import streamlit as st
from datetime import datetime
from io import BytesIO

ACTIVE_ENGAGEMENT_ID_KEY = "active_engagement_id"
ACTIVE_ENGAGEMENT_NAME_KEY = "active_engagement_name"
...
def render_engagement_selector(page_key):
    ...
    if not engs.empty and "status" in engs.columns:
        ...
        if not open_engs.empty:
            engs = open_engs
    if engs.empty:
        st.info("No audit engagements found. Create one in Audit Session Manager first.")
        st.session_state[eng_key] = None
        st.session_state[eng_name_key] = None
        st.session_state[ACTIVE_ENGAGEMENT_ID_KEY] = None
        st.session_state[ACTIVE_ENGAGEMENT_NAME_KEY] = None
        return
    eng_options = {}
    ...
    global_id = st.session_state.get(ACTIVE_ENGAGEMENT_ID_KEY)
    if current_id not in ids_list and global_id in ids_list:
        current_id = global_id
        st.session_state[eng_key] = current_id
        st.session_state[eng_name_key] = eng_options[current_id]
    ...
    if selected_id != current_id:
        ...
        st.session_state.setdefault(eng_key, selected_id)
        st.session_state.setdefault(eng_name_key, eng_options[selected_id])

    st.session_state[ACTIVE_ENGAGEMENT_ID_KEY] = selected_id
    st.session_state[ACTIVE_ENGAGEMENT_NAME_KEY] = eng_options[selected_id]
    st.caption(
...
def get_active_engagement_id(page_key):
    ...
    return st.session_state.get(page_key + "_engagement_id") or st.session_state.get(ACTIVE_ENGAGEMENT_ID_KEY)
...
def render_draft_review_section(page_key, module_name):
    ...
    Handles multiple run_ids (for pages with multiple detection types like GST/TDS).
    Loads all draft findings for the module regardless of run_id.
    """
    import pandas as pd
    ...
    # Try to get the primary run_id from session state
    # Also handle pages that have multiple run_ids (like gst_gst_run_id, gst_tds_run_id)
    run_ids_to_try = [
        st.session_state.get(page_key + "_draft_run_id"),
        st.session_state.get(page_key + "_gst_run_id"),
        st.session_state.get(page_key + "_tds_run_id"),
        st.session_state.get(page_key + "_run_id"),
    ]
    run_ids_to_try = [r for r in run_ids_to_try if r is not None]
    
    # Default view should show current-run drafts only, otherwise it looks like
    # exceptions are auto-staged on page load due to historical pending drafts.
    engagement_id = get_active_engagement_id(page_key)
    active_run_id = run_ids_to_try[0] if run_ids_to_try else None
    show_history = st.checkbox(
        "Show historical pending drafts for this module",
        value=False,
        key=page_key + "_show_history_drafts",
    )
    if active_run_id and not show_history:
        ...
        drafts = load_draft_findings(module_name=module_name, status="Draft", engagement_id=engagement_id)
    else:
        drafts = pd.DataFrame()
    
    if drafts.empty:
        if not active_run_id and not show_history:
            st.info("No staged exceptions for the current run yet. Upload a file and run detection.")
        else:
            # Check for any confirmed findings for this module
            ...
            if not already_confirmed.empty:
                st.caption(
                    str(len(already_confirmed))
                    + " exception(s) for this module are already confirmed for the selected engagement."
                )
            else:
                st.info("No staged exceptions found. Upload a file and run detection first.")
    else:
        st.caption("**" + str(len(drafts)) + " draft exception(s)** pending review for **" + module_name + "**")
        
        # Show summary of findings by area
        if "area" in drafts.columns:
            area_labels = drafts["area"].fillna("").astype(str).str.strip().replace("", "Unspecified")
            area_summary = area_labels.groupby(area_labels).size()
            st.caption("Areas: " + " | ".join([str(a) + ": " + str(c) for a, c in area_summary.items()]))
        
        # Use data_editor for editable findings (same pattern as anomaly_detector.py)
        editable_cols = ["id", "vendor_name", "finding", "amount_at_risk", "risk_band"]
        available_cols = [c for c in editable_cols if c in drafts.columns]
        review_df = drafts[available_cols].copy()
        
        column_config = {
            "id": st.column_config.NumberColumn("ID", disabled=True, width="small"),
            "vendor_name": st.column_config.TextColumn("Vendor", disabled=True) if "vendor_name" in available_cols else None,
        ...
        id_labels = {
            row["id"]: "ID " + str(row["id"]) + " -- " + str(row.get("vendor_name", "N/A")) + " (" + str(row.get("area", "N/A")) + ")"
        ...
        with c_confirm:
            if st.button(
                ...
                type="primary",
                use_container_width=True,
                key=page_key + "_confirm_btn"
            ):
                if not selected_ids:
                    st.warning("Select at least one exception to confirm.")
                else:
                    # Apply any edits from data_editor
                    edited_vals = {
                        int(row["id"]): {
                    ...
                    st.success("**" + str(n) + " finding(s) confirmed** and added to the official audit trail. They will now appear in Audit Report Center and all reports.")
                    st.rerun()
        
        with c_discard:
            discard_reason = st.text_input(
                "Discard reason (optional)",
                key=page_key + "_discard_reason"
            )
            if st.button(
                "Discard Selected (False Positives)",
                use_container_width=True,
                key=page_key + "_discard_btn"
            ):
                if not selected_ids:
                    ...
                    st.info("**" + str(n) + " exception(s) discarded.** They will not appear in reports.")
                    st.rerun()
        ...
        st.download_button(
            ...
            csv_draft,
            "draft_exceptions_" + page_key + ".csv",
            "text/csv",
            key=page_key + "_export_drafts"
        )
