...
if uploaded_file:
    ...
    min_amt, max_amt = st.sidebar.slider("Amount Range", float(df['amount'].min()), float(df['amount'].max()), (float(df['amount'].min()), float(df['amount'].max())))
    categories = st.sidebar.multiselect("Category", df['category'].unique(), default=df['category'].unique())
    show_related = st.sidebar.checkbox("Show Only Related Party", False)
    min_overdue, max_overdue = st.sidebar.slider("Days Overdue", int(df['days_overdue'].min()), int(df['days_overdue'].max()), (int(df['days_overdue'].min()), int(df['days_overdue'].max())))
    ...
    with tab1:
        ...
        fig_scatter = px.scatter(filtered_df, x='amount', y='Risk_Score', color='Anomaly', color_discrete_map={-1:'red',1:'lime'}, hover_data=['vendor_name', 'transaction_id', 'category'])
    ...
    st.info("Upload vendor payments CSV from sidebar")